...
## Editor's Notes (/book-edit session — 2026-06-03)

### What Changed and Why
- **Em-dash removal (global):** All em-dashes replaced with periods, colons, or commas. Em-dashes have become a fingerprint of AI prose; readers react against them viscerally. Each replacement forced a more precise punctuation decision: period where ideas break, colon where one elaborates another, comma where ideas continue.
- **What Got Installed rewrite:** Cut the abstract "Every man has a pattern..." paragraph — the grandfather/father/author story does that work better. Grandfather portrait updated: removed anachronistic "ride-or-die"; added era-appropriate dominance frame ("undisputed head of that household," screaming read as authority). Mother portrait corrected from pure withdrawal to the actual dual pattern: silence plus precision strikes ("She knew where it would hurt most"). Author's own pattern updated to include both sulking and biting language ("I can find that sentence").
- **Equanimity defined:** Added inline plain-English definition at first use: "the Stoic discipline of staying steady without going cold."

### Author's Notes
- Grandfather: devotion was never in question; screaming came from dominance/authority, not instability. Era: 1950s household structure.
- Mother: not just withdrawal — withdrawal plus surgical biting language. More nuanced than the previous draft.
- Author's pattern: sulking + precision strikes, not yelling or physical.

### Remaining Placeholder Count
- **0** `[PLACEHOLDER]` items
- **0** `[STORY NEEDED]` items

### Structural Flag
None. Previous version backed up to `refined-prev.md`.

---

## Editor's Notes (/book-chapter-refine session — 2026-06-19)

### Why this pass ran
Reader craft feedback on the Ch1 Substack post ("The Operating System") surfaced
two counted-rule violations in `01-voice.md` that were new at the time this
chapter was last refined (2026-06-03): metaphor word-family saturation (Cat H)
and rhetorical-device repetition (Cat I in the original five, now Cat J in the
hardened anti-slop spec). Since both rules are book-wide, not Substack-only,
this chapter was checked against them directly, with the same literal-count
verification standard (`01-voice.md`, "Verification, Not Impression") applied
to the Substack fix.

### Pass 4 — Anti-Slop, Cat G–J (counted, verified by recount)
- **Cat H — Metaphor saturation.** Initial count: 12 mentions of the
  operating-system word family (`operating system`, `installed` x2, `run`/
  `running`/`runs` x9, `defaults`) against ~1,984 body words — a cap of
  roughly 6 at 3/1,000 words. Most of the excess came from unrelated uses of
  "run"/"running" (running the tape back, the running score, running an
  empire) that aren't part of the operating-system conceit but still count
  toward the literal family per the hardened rule. Replaced six non-anchor
  instances with words outside the family (replaying, happened, live,
  go straight through, governing, the score you've been keeping). Final
  verified count: 6 — at the cap, with the anchor mentions (`operating
  system` x1, `installed` x2, the core "Run that sequence" line, "runs on
  its own," `defaults`) preserved.
- **Cat J — Rhetorical device repetition (negated comparison / reframe
  shape: "X isn't/wasn't that. It's/He's Y.").** Initial count, by literal
  re-scan: 3 instances — the personal confession ("I don't yell. I don't
  break things. But..."), the equanimity reframe ("What I'd done wasn't
  that. It was an excuse to disappear..."), and an unflagged third instance
  in the replay-question section ("isn't a checklist to use mid-fight. It's
  what you walk through afterward") that read as fine on a holistic pass but
  matched the same sentence-shape on count. Rewrote the third instance into
  a flat declarative ("Save this for afterward, not mid-fight. Walk through
  it then.") and varied two adjacent sentences in the equanimity paragraph
  that had independently drifted into the same shape. Final verified count:
  2 — at the cap.
- **Cat G (bold) and Cat I (double endings):** checked, no violations. The
  bolded lines are structural (section headers, the four-virtue list labels)
  rather than emphasis-as-crutch on a single sentence, which the voice spec
  treats differently. The closing line ("Use it.") is a single sentence with
  nothing following it.

### Scope of this pass
This was a targeted recheck against the newly hardened counted rules, not a
full four-pass rewrite — the chapter's approachability, voice, clarity, and
flow were spot-checked with no new findings since the 2026-06-03 pass.

### Remaining Placeholder Count
- **0** `[PLACEHOLDER]` items
- **0** `[STORY NEEDED]` items

### Citation corrections (2026-09-09 12:56)
...
- *Discourses* II.18: the line was a paraphrase printed inside quotation marks with a citation (external verification, 2026-09-06). Replaced with George Long's 1877 wording, transcribed from the Project Gutenberg text (#10661) via its GitHub mirror. One clause of prose adjusted so "appearances," Long's word, lands for a reader who was just told the Stoic word is "impression."
