...
land() gates on status()'s behind_main/ahead_of_main, both computed against origin/main (sync.py L78-79), then runs 'git checkout main' and 'git merge --ff-only <branch>' against LOCAL main (L162-163). Nothing asserts the two are the same ref. On 2026-09-19 --status reported '6 ahead, 0 behind' and --land then died on 'fatal: refusing to merge unrelated histories' - confirmed: the original engine root c74fe66 is NOT an ancestor of origin/main, i.e. local main and origin/main were genuinely unrelated histories at that moment. [Corrected 2026-09-20: the two sentences that used to sit here, naming a specific forced-update SHA and claiming local main "stayed on a history the remote no longer has" as the cause, were wrong on both counts - checked three times and wrong a different way each time - and are cut rather than attempted a fourth. The reporting defect does not need the story of how the 2026-09-19 incident happened: every guard in land() measures origin/main but acts on local main, unconditionally.] Same shape sync.py already records against itself at L50-52 (a false zero with no error, in the one script whose job it is to prevent that).
